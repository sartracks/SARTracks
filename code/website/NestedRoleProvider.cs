...
namespace SarTracks
{
    ...
    public class NestedRoleProvider : SqlRoleProvider
    {
        public static ExtendedRole GetRole(string roleName)
        {
            return (Roles.Provider is NestedRoleProvider)
                ? ((NestedRoleProvider)Roles.Provider).ExtendedGetRole(roleName)
        ...
        {
            return (Roles.Provider is NestedRoleProvider)
                ? ((NestedRoleProvider)Roles.Provider).ExtendedGetAllRoles()
        ...
        public void UpdateRole(ExtendedRole role, string originalRole)
        {
            using (SqlConnection connection = new SqlConnection(this.connectionString))
            {
                SqlCommand cmd = new SqlCommand("UPDATE aspnet_Roles SET RoleName=@newname, owners=@owners, emailaddress=@email, externaldest=@destinations WHERE RoleName = @name", connection);
